...
runner 实际循环处理全部 3,122,000 个合成代理，并只保留分组、区域、方式、路线模板、时间直方图和总量校验；不落个人地址、雇主、身份或连续轨迹。B0 是集中到达的压力场景，O1 是企业弹性到岗、公共交通优先、连续无障碍和路缘治理的整体优化候选，R1 是地铁中断后的地面回退屏查。优化顺序是先过消防/应急、无障碍、公共交通保护、隐私、人工服务和空中候选关闭等硬门，再比较广义成本、p90 时间、人流冲突、外部汽车流入、车辆公里和分组差距；输出的“满意度”是由时间、可靠性、可达性和冲突构成的 `satisfaction_proxy`，不是居民调查结果。

在当前合成输入下，O1 的满意度代理为 54.67 → 66.23，广义成本代理为 60.34 → 50.06，p90 时间代理为 90 → 60 分钟，人流冲突代理为 5.74 → 3.01/千人，外部汽车流入为 26.95% → 8.60%。这些读数只证明模型能在区域人口规模上守恒、回算和比较，不证明海淀现状已获得同样改善；真正决策前仍需补分时 OD、工作人口结构、站点/公交/路缘容量、分组方式份额、门到门 p50/p90、无障碍审计和居民/员工满意度调查。
