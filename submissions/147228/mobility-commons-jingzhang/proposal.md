...
runner 实际循环处理全部 3,122,000 个合成代理，并只保留分组、区域、方式、路线模板、时间直方图和总量校验；不落个人地址、雇主、身份或连续轨迹。B0 是集中到达的压力场景，O1/O2/O3/O4 是公共交通、慢行、公平和容量平衡的四组候选，R1 是地铁中断后的地面回退屏查。runner 对 O1/O2/O3/O4 都做全量回放，先执行包含峰值方式负荷 ≤1.35× 的硬门，再按“满意度代理最大 → 广义成本 → p90 → 冲突 → 外部汽车流入 → 车辆公里”的字典序选择；当前合成输入下 O3 虽然纸面满意度最高，但自行车峰值负荷达 1.57× 被硬门排除，最终由 O4 容量平衡候选胜出。输出的“满意度”是由时间、可靠性、可达性和冲突构成的 `satisfaction_proxy`，不是居民调查结果。
