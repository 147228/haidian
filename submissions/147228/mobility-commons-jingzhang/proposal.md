...
runner 实际循环处理全部 3,122,000 个合成代理，并只保留分组、区域、方式、路线模板、时间直方图和总量校验；不落个人地址、雇主、身份或连续轨迹。B0 是集中到达的压力场景，O1/O2/O3 是三组不同的公共交通、慢行和公平权重候选，R1 是地铁中断后的地面回退屏查。runner 对 O1/O2/O3 都做全量回放，按“硬门 → 满意度代理最大 → 广义成本 → p90 → 冲突 → 外部汽车流入 → 车辆公里”的字典序选择，当前合成输入下由 O3 慢行优先候选胜出；输出的“满意度”是由时间、可靠性、可达性和冲突构成的 `satisfaction_proxy`，不是居民调查结果。

在当前合成输入下，全量搜索选中的 O3 满意度代理为 54.67 → 66.78，广义成本代理为 60.34 → 48.93，p90 时间代理为 90 → 60 分钟，人流冲突代理为 5.74 → 3.11/千人，外部汽车流入为 26.95% → 6.30%，车辆公里代理为 28.11M → 26.00M。O1/O2/O3 的候选排名和硬门结果都写入 runner 输出，避免把一个手选方案冒充“最优”。这些读数只证明模型能在区域人口规模上守恒、回算和比较，不证明海淀现状已获得同样改善；真正决策前仍需补分时 OD、工作人口结构、站点/公交/路缘容量、分组方式份额、门到门 p50/p90、无障碍审计和居民/员工满意度调查。
