...
- 针对榜首固定图暴露的规划深度差距，把 `geometry/land_use.geojson` 从 4 条粗带细化为 8 个无重叠、完整覆盖 provisional site 的标准用地分类多边形，并为每块写入可复核面积与设计占比。
- 五张双语首屏图同步重绘：增加真实边界形态、8 类用地、北向、三处站点、三条东西连接、核心命题、明确入口/人工闸门/输出/停止条件和指标卡；不把设计占比升级为法定控规。
- 图册、HTML、版权台账与 manifest 将同步更新为 v3.2；继续保留 official polygon 到位后重算的边界声明。
- 根据评审复读完成权利清洁修复：移除带 peer-derived 结构的 Relay Receipt schema 与样例，换成独立设计的有界测试窗口记录；主稿与离线 HTML 记录该历史修正，避免把署名误作改编许可。
- 将历史附图及其 A3/A0 内嵌副本的页脚从 v3.0 统一为 v3.2，并重新计算双语图件、图册、版权台账和 manifest 哈希。
