...
runner 实际循环处理全部 3,122,000 个合成代理，并只保留分组、区域、方式、路线模板、时间直方图和总量校验；不落个人地址、雇主、身份或连续轨迹。B0 是集中到达的压力场景，O1/O2/O3/O4 是公共交通、慢行、公平和容量平衡的四组候选，R1 是地铁中断后的地面回退屏查。runner 对 O1/O2/O3/O4 都做 AM 全量回放，先执行包含峰值方式负荷 ≤1.35× 的硬门，再按“满意度代理最大 → 广义成本 → p90 → 冲突 → 外部汽车流入 → 车辆公里”的字典序选择；当前合成输入下 O3 虽然纸面满意度最高，但自行车峰值负荷达 1.57× 被硬门排除，最终由 O4 容量平衡候选胜出。选中的 O4 随后再对全部 3,122,000 人做独立 PM 返程覆盖屏查，反向聚合工作/服务 → 居住路线；这不是观测到的晚高峰 OD。输出的“满意度”是由时间、可靠性、可达性和冲突构成的 `satisfaction_proxy`，不是居民调查结果。

在当前合成输入下，全量搜索选中的 O4 满意度代理为 54.67 → 66.44，广义成本代理为 60.34 → 49.44，p90 时间代理为 90 → 60 分钟，人流冲突代理为 5.74 → 3.13/千人，外部汽车流入为 26.95% → 8.47%，车辆公里代理为 28.11M → 25.94M，峰值方式负荷由 1.57× 降至 1.21×。O1/O2/O3/O4 的候选排名、容量负荷和硬门结果都写入 runner 输出，避免把一个手选方案冒充“最优”；O3 的更高满意度只有在补足自行车容量后才可重新进入候选。上述读数只证明模型能在区域人口规模上守恒、回算和比较，不证明海淀现状已获得同样改善；真正决策前仍需补分时 OD、工作人口结构、站点/公交/路缘容量、分组方式份额、门到门 p50/p90、无障碍审计和居民/员工满意度调查。
