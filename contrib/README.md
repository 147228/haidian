...
- **离群值只给计数**：单位缺失/null/非字符串/空字符串与非法单位分开计数（`unit_missing` / `unit_not_in_enum`），不把数据缺口冒充枚举错误，也不指出具体包。
