...
在不挪动居民、照护者/儿童、访客、物流维护和夜班人员需求的前提下，B1 只对 450 个企业员工设计单元做 20% 的弹性到岗敏感性，把 90 个设计单元从高峰移入更宽的到岗窗口。离线 runner 的合成读数为：总峰值排队 174 → 164，平均排队人分钟 75.7999 → 71.7917；汽车路缘峰值排队保持 60，未满足需求保持 89。地铁、公交负载下降只是这个声明输入下的压力变化，不是企业真实响应或海淀现状绩效；正式使用前必须用有日期的企业分组 OD、方式份额、班次和站口计数替换输入 [source:MATRAM-ACTIVITY-ADAPTATION-2026] [metric:mode_transfer_reliability]。
